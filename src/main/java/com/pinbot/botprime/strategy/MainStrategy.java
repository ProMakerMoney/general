...
/**
 * MainStrategy (без фильтра боковика):
 *  - Входы/окна как раньше.
 *  - TP1 = 2R (50% объёма), затем стоп остатка = BE, TP2 = финальный выход.
 *  - RSI-выходы только на 2h (сравнение i-4 и i). Порог: 25/75.
 *
 *  - Стоп (как в FirstStrategy):
 *      * Окно импульсов: [entryIndex-5 .. entryIndex] (6 баров).
 *      * Если импульса нет → TEMA9 (LONG = min, SHORT = max) в этом окне.
 *      * Если импульс есть → кандидаты уровней:
 *          - EMA110 и EMA200 на баре импульса,
 *          - уровни кросса EMA11/EMA30 в окне ((ema11+ema30)/2 на баре кросса).
 *        Для LONG — сначала нижняя половина тела импульса (zone1), затем расширение вниз (zone2);
 *        для SHORT — сначала верхняя половина (zone1), затем расширение вверх (zone2).
 *        Если кандидатов нет вовсе → fallback на TEMA9.
 *        Если кандидаты есть:
 *          - Если разница между min и max < 0.03% к среднему — LONG берём min, SHORT — max.
...
public class MainStrategy {
    ...
    public List<MainBacktestTrade> backtest(List<Bar> bars) {
        ...
        for (int i = 0; i < bars.size(); i++) {
            ...
            if (pos != null) {
                ...
                // 2.3) RSI-выход только на 2h (сравнение i-4 и i), пороги 25/75
                if (!positionClosedThisBar && i >= 4 && is2hBoundary(b.openTime())) {
                    ...
                    if (rPrev != null && sPrev != null && r != null && s != null) {
                        if (pos.dir == Dir.LONG) {
                            if (!pos.armed75 && r.compareTo(RSI_HIGH) >= 0) pos.armed75 = true;
                            boolean crossDown = rPrev.compareTo(sPrev) > 0 && r.compareTo(s) <= 0;
                            boolean armed75Ex = pos.armed75 && r.compareTo(RSI_HIGH) < 0;
                            if (crossDown || armed75Ex) {
                                trades.add(buildTrade(pos, nextOpenTime(b), scale2(b.close()),
                                        crossDown ? "RSI_CROSS" : "RSI_75_25"));
                                pos = null; positionClosedThisBar = true;
                            }
                        } else {
                            if (!pos.armed25 && r.compareTo(RSI_LOW) <= 0) pos.armed25 = true;
                            boolean crossUp   = rPrev.compareTo(sPrev) < 0 && r.compareTo(s) >= 0;
                            boolean armed25Ex = pos.armed25 && r.compareTo(RSI_LOW) > 0;
                            if (crossUp || armed25Ex) {
                                trades.add(buildTrade(pos, nextOpenTime(b), scale2(b.close()),
                                        crossUp ? "RSI_CROSS" : "RSI_75_25"));
                                pos = null; positionClosedThisBar = true;
                            }
                        }
                    }
            ...
            // 7) Планируем вход на i+1 (без фильтра боковика)
            if (entryDir != null) {
                int entryIndex = i + 1;
    ...
    private StopCalcResult computeStopForEntry(List<Bar> bars, int entryIndex, Dir dir) {
        ...
        return new StopCalcResult(chosenLevel, source, impulse);
    }
    ...
    /** <— ЭТОГО метода не хватало в твоей сборке */
